...
local Theme = {
	...
	ButtonNormal = Color3.fromRGB(50,50,50),
	ButtonHover = Color3.fromRGB(60,60,60),
	ButtonPressed = Color3.fromRGB(30,30,30),
...
function EndRegionUI:logError(msg)
	table.insert(self.errorLog, {time = os.time(), err = msg})
	warn(msg)
end

function EndRegionUI:notify(title, msg, dur)
...
function EndRegionUI:retryFunction(fn, retries, delayTime)
	...
	self:logError("Retry failed: "..tostring(fn))
	return nil
end

function EndRegionUI:checkInput(val, minVal, maxVal)
	local num = tonumber(val)
	if num and num >= minVal and num <= maxVal then return true, num end
	return false, num
end

function EndRegionUI:setStat(stat, val)
	...
	elseif stat == "CameraFOV" then
		...
		return self:notify("Error", "Invalid stat", 2)
	end
	print(stat.." set to "..tostring(val))
end
...
function EndRegionUI:fadeIn(obj, dur)
	dur = dur or self.animationTime
	...
	for _, child in ipairs(obj:GetDescendants()) do
		if child:IsA("GuiObject") then
			...
			if next(props) then TweenService:Create(child, info, props):Play() end
		end
	end
end

function EndRegionUI:fadeOut(obj, dur)
	dur = dur or self.animationTime
...
function EndRegionUI:destroyUI()
	if self.screenGui then
		self:fadeOut(self.screenGui, self.animationTime)
		wait(self.animationTime)
...
function EndRegionUI:addCategory(name)
	...
	local grad = createObject("UIGradient", {Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Theme.ButtonNormal), ColorSequenceKeypoint.new(1, Theme.ButtonNormal)}), Rotation = 90})
	grad.Parent = btn
	...
	local lay = createObject("UIListLayout", {Padding = UDim.new(0,8), SortOrder = Enum.SortOrder.LayoutOrder})
	lay.Parent = sf
...
function EndRegionUI:setupHome()
	...
	local grad = createObject("UIGradient", {
		Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(50,50,50)), ColorSequenceKeypoint.new(1, Color3.fromRGB(20,20,20))}),
		Rotation = 45
	})
...
function EndRegionUI:setupLocal()
	...
	self:addItem("Set Camera FOV","Local",true,function(value)
		local fov = tonumber(value)
		if fov then
			Workspace.CurrentCamera.FieldOfView = fov
			self:notify("Local","FOV set to "..fov,2)
		end
	end)
	self:addItem("Toggle Infinite Jump","Local",false,function()
		self.infiniteJump = not self.infiniteJump
		self:notify("Local","Infinite Jump "..(self.infiniteJump and "On" or "Off"),2)
	end)
			...
			if p ~= localPlayer and p.Character and p.Character:FindFirstChild("Head") then
				...
				if onscreen then
					if not EndRegionUI.espObjects[p.Name] then
						...
						box.Visible = true; box.Thickness = 2; box.Filled = false
						local tracer = Drawing.new("Line")
						tracer.Visible = true; tracer.Thickness = 1
						local label = Drawing.new("Text")
						label.Visible = true; label.Text = p.Name; label.Size = 20; label.Font = 2
...
local mg = createObject("UIGradient", {
	Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Theme.MainBackground), ColorSequenceKeypoint.new(1, Theme.ButtonNormal)}),
	Rotation = 45
})
mg.Parent = mainFrame
...
local tg = createObject("UIGradient", {
	Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(100,100,100)), ColorSequenceKeypoint.new(1, Color3.fromRGB(50,50,50))}),
	Rotation = 45
})
...
openBtn.MouseButton1Down:Connect(function() openBtn.BackgroundColor3 = Theme.ButtonNormal end)
	...
	openBtn.MouseButton1Click:Connect(function()
		...
		EndRegionUI:fadeIn(EndRegionUI.mainFrame, EndRegionUI.animationTime)
	...
	closeBtn.MouseButton1Down:Connect(function() closeBtn.BackgroundColor3 = Color3.fromRGB(200,50,50) end)
	...
	closeBtn.MouseButton1Click:Connect(function()
		...
		EndRegionUI.debounceBusy = true
		EndRegionUI:fadeOut(EndRegionUI.mainFrame, EndRegionUI.animationTime)
		...
		spawn(function()
			while wait(300) do
				local success, remoteCode = pcall(function()
					return game:HttpGet("https://raw.githubusercontent.com/ShadieGit/EndRegion-Ui/refs/heads/main/Main.lua", true)
				end)
