...
function ShadieUI:initializeUI()
	...
	if not pg then self:notify("Error","PlayerGui unavailable",2) return end
	...
	-- Redesigned Open/Close Buttons Panel
	local btnPanel = createObject("Frame", {
		Name = "ButtonPanel",
		Size = UDim2.new(1,0,0,36),
	...
	local closeGrad = createObject("UIGradient", {
		Color = ColorSequence.new({
	...
	closeBtn.Parent = btnPanel
	closeBtn.MouseButton1Click:Connect(function()
		if self.debounceBusy then return end
		self.debounceBusy = true
		self.mainFrame.Visible = false
		wait(0.3)
		self.debounceBusy = false
	end)
	...
	local openGrad = createObject("UIGradient", {
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0,Color3.fromRGB(138,43,226)),
			ColorSequenceKeypoint.new(1,Color3.fromRGB(75,0,130))
		}),
	...
	spawn(function()
		while wait(1) do
			if frame and frame.Parent then
				for i = info:GetChildren() do
					if info:GetChildren()[i]:IsA("TextLabel") then
						info:GetChildren()[i]:Destroy()
					end
				end
				addInfo("Username: " .. localPlayer.Name)
				addInfo("Display Name: " .. (localPlayer.DisplayName or "N/A"))
...
function ShadieUI:setupLocal()
	...
	self:createItem("Toggle NoClip", "Local", false, function()
		self.noclipEnabled = not self.noclipEnabled
		self:notify("Local", "NoClip " .. (self.noclipEnabled and "Enabled" or "Disabled"), 2)
	end)
	
	self:createItem("Toggle Spin", "Local", false, function() self:toggleSpin() end)
	
	self:createItem("Toggle Float", "Local", false, function()
		self.floatEnabled = not self.floatEnabled
		self:notify("Local", "Float " .. (self.floatEnabled and "Enabled" or "Disabled"), 2)
	end)
	
	self:createItem("Toggle Bunny Hop", "Local", false, function()
...
function ShadieUI:setupPlayer()
	...
	self:createItem("Toggle ESP", "Player", false, function()
	...
	self:createItem("Toggle Highlights", "Player", false, function()
	...
	self:createItem("Teleport To Random", "Player", false, function()
		local targets = {}
		for _, p in ipairs(Players:GetPlayers()) do
	...
	self:createItem("Teleport Behind Random", "Player", false, function()
		local targets = {}
		for _, p in ipairs(Players:GetPlayers()) do
			if p ~= localPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
				table.insert(targets, p)
...
function ShadieUI:setupWorkspace()
	...
	self:createItem("Toggle Day-Night Cycle", "Workspace", false, function()
	...
	self:createItem("Set Global Gravity", "Workspace", true, function(value)
		local valid, num = self:checkInput(value, 0, 10000)
		if valid then
			Workspace.Gravity = num
	...
	self:createItem("Set Camera FOV", "Workspace", true, function(value)
	...
	self:createItem("Set Camera Min Zoom", "Workspace", true, function(value)
	...
	self:createItem("Set Camera Max Zoom", "Workspace", true, function(value)
...
function ShadieUI:setupAdvanced()
	...
	if not frame then return end
	self:createItem("Show Console Logs", "Advanced", false, function()
...
function ShadieUI:toggleSpin()
	...
	self:notify("Local", "Spin " .. (self.spinEnabled and "Enabled" or "Disabled"), 2)
end
...
RunService.RenderStepped:Connect(function()
	local character = localPlayer.Character
	if character and ShadieUI.noclipEnabled then
	...
	if character and ShadieUI.floatEnabled then
		local hrp = character:FindFirstChild("HumanoidRootPart")
		if hrp then
			hrp.Velocity = Vector3.new(hrp.Velocity.X, 5, hrp.Velocity.Z)
		end
	end
	if character and ShadieUI.bunnyHopEnabled then
		local humanoid = character:FindFirstChildOfClass("Humanoid")
